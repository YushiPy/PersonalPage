...
class Scene {
	 ...
	 * @param {{ includePreview?: boolean }} [options] — If `includePreview` is false (e.g. file export), omits PNG thumbnail and canvas dimensions for a smaller JSON.
	 */
	saveData(drawingName, options = {}) {
		...
		if (includePreview) {
			data.dataURL = scene.canvas.canvas.toDataURL("image/png");
	...
	async saveToDB(drawingName) {
		...
		return drawingStore.createDrawing(data);
	}

	async updateDrawingData(drawingId, drawingName) {
		const data = this.saveData(drawingName);
		await drawingStore.updateDrawing(drawingId, data);
	}
